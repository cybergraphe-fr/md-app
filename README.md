...
| **PDF orientation** | Portrait (default) or Landscape (A4) — selectable in export modal; margins and pagination are preserved |
...
| `POST` | `/api/files/:id/export/:format` | Export (pdf, docx, odt, epub, latex, rst, asciidoc, textile, mediawiki, plain). PDF accepts `?margin=standard\|narrow\|wide` or `?mt=&mr=&mb=&ml=` (cm), optional `?orientation=portrait\|landscape` (default portrait), optional `?header=` and `?footer=`, optional `?header_align=left\|center\|right`, `?footer_align=left\|center\|right`, and `?h1_underline_color=%23RRGGBB`. Heading style options are available on rich exports via `?heading_text_color=%23RRGGBB` (default `#111111`), `?heading_font=sans\|serif\|mono`, plus explicit typography params `?heading_font_name=<FontName>` and `?body_font_name=<FontName>` for PDF/web font fidelity (for example `Tangerine` + `Lora`). H2 can be customized separately with `?h2_text_color=%23RRGGBB` and `?h2_underline_color=%23RRGGBB`. |
| `POST` | `/api/files/import` | Import via multipart form (`file` field) |
| `POST` | `/api/export/raw/:format` | Export raw content without saving `{content, name}`. PDF accepts same margin params, `orientation`, and optional layout params (`header`, `footer`, `header_align`, `footer_align`, `h1_underline_color`). Rich exports also accept `heading_text_color`, `heading_font`, `heading_font_name`, and `body_font_name` for heading/body typography customization, plus `h2_text_color` and `h2_underline_color` for H2-specific styling. |
